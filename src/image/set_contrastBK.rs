...
impl Image {
    ...
    // 用PDF得出一个最优值, 页面一打开, 就显示在slidebar上, 让用户选择(左右拉动), 但其中有个值labeled as "optimal'.
    ...
    pub fn set_contrast(&mut self) {
        ...
        // 4 items denote 4 RGBA color channels
        ...
        for row in 0..h {
            for col in 0..w {
                ...
                new_pixel_value = intensity_dist[0][pixel_value as usize] * 255 as f64;
                ...
                new_pixel_value = intensity_dist[1][pixel_value as usize] * 255 as f64;
                if new_pixel_value > 255.0 {
                ...
                new_pixel_value = intensity_dist[2][pixel_value as usize] * 255 as f64;
                if new_pixel_value > 255.0 {
                ...
                new_pixel_value = intensity_dist[3][pixel_value as usize] * 255 as f64;
                if new_pixel_value > 255.0 {
                    self.pixels[pixel_idx * 4 + 3] = 255
                } else {
                    self.pixels[pixel_idx * 4 + 3] = new_pixel_value.round() as u8
                }
